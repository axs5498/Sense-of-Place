...
age_65to75 <- age[,6]
age_ge75 <- age[,7]
age_le45 <- abs(rowSums(cbind(age[,1],age[,2],age[,3])))
age_45to65 <- abs(rowSums(cbind(age[,4],age[,5])))
...
hhmem_2ormore<-abs(rowSums(cbind(hhmem_1))-1)
hhmem_3ormore<-abs(rowSums(cbind(hhmem_1,hhmem_2))-1)
...
#newvariable
emp_2ormore<-abs(rowSums(cbind(emp_ft[,1],emp_ft_2))-1)
emp_1ormore<-abs(emp_ft_0-1)
...
inc_75kto100k<-inc[,9]
inc_DtoA<-inc[,10]
inc_ge100k<-cbind(inc_100kto125k,inc_125kto150k,inc_175kto200k,inc_ge200k) #generating new bins 
inc_ge100k<-rowSums(inc_ge100k)
...
twiceaweek<-visit[,8]
#weekly<-rowSums(cbind(everyday,onceaweek,twiceaweek,threeormoretimesaweek)) #generating new bins 
morethanonceaweek<-rowSums(cbind(everyday,twiceaweek,threeormoretimesaweek,everyday))
oneweekto1month<-abs(rowSums(cbind(onceaweek,onceamonth,onceinthreeweeks,onceintwoweeks)))
atleastonceamonth<-abs(rowSums(cbind(leonceamonth))-1)
...
#generating new bins
biwalk.yes<-abs(biwalk.none-1) #generating new bins 0
biwalk.gtonceamonth<-rowSums(cbind(biwalk.none,biwalk.lemonth,biwalk.oncemonth)) #generating new bins
biwalk.gtonceamonth<-abs(biwalk.atleastonceamonth-1) #generating new bins
...
#dataset for analysis 
newds<-workds_NA4[,sat_RPM]
newds<-cbind(newds,age_35to45,hhmem_2, emp_2ormore)
             #emp_ft_0,emp_ft_1, emp_ft_2, emp_ft_3, emp_ft_4, emp_ft_5ormore, emp_ft_DtoA

newds<-newds[1:134,]

model.RPM.sat.empft<-specifyModel("model_RPM_sat_empft.R") 
#fitting the model 
...
#dataset for analysis 
newds<-workds_NA4[,sat_RPM]
newds<-cbind(newds,age_35to45,hhmem_2, emp_ft_1, rltnHHmem_family, morethanonceaweek)
...
#dataset for analysis 
newds<-workds_NA4[,att_RPM]
newds<-cbind(newds,age_55to64,age_45to54,notfromRoc,hhmem_5ormore,emp_ft_3,weekly)
             #everyday, #onceamonth, #onceaweek,# onceinthreeweeks, # onceintwoweeks, 
             #threeormoretimesaweek, # twiceaweek, #leonceamonth
...
###Aesthetics - Income
Significant variables *inc_100kto125k,inc_125kto150k*. They were converted into a single variables *inc_ge100k* which was significant
...
#dataset for analysis 
newds<-workds_NA4[,aes_RPM]
newds<-cbind(newds,female,inc_ge100k)
             #inc_75kto100k,inc_ge200k,inc_50kto75k,inc_25kto50k,inc_175kto200k,
            #inc_100kto125k,inc_125kto150k,inc_150kto175k, inc_DtoA,inc_le25k) 
...
#dataset for analysis 
newds<-workds_NA4[,aes_RPM]
newds<-cbind(newds,female,inc_100kto125k,inc_125kto150k,rltnHHmem_acquaintances,weekly)
             #everyday, #onceamonth, #onceaweek,# onceinthreeweeks, # onceintwoweeks, 
             #threeormoretimesaweek, # twiceaweek, #leonceamonth
...
#dataset for analysis 
newds<-workds_NA4[,aes_RPM]
newds<-cbind(newds,female,inc_ge100k,rltnHHmem_acquaintances,onceintwoweeks,biwalk.oncemonth)
RPM.aes.vars<-c("female","inc_ge100k","rltnHHmem_acquaintances","onceintwoweeks","biwalk.oncemonth")             
newds<-newds[1:134,]
...
#income
inc_ge100k->aes,fi22a, NA
inc_ge100k<-> inc_ge100k,err34a, NA
